...
class BALL(tk.Frame):
    def __init__(self, parent, canvas, ball, walls, speed=10):
        ...
        # Defaul vals
        self.entry_X.insert(0, 300)
        self.entry_Z.insert(0, 50)
        self.entry_Xv.insert(0, -500)
        self.entry_Zv.insert(0, 200)
        self.entry_g.insert(0, -500)
...
if __name__ == "__main__":
    colors_dcit = {2.0: "darkblue", 1.5: "blue", 1.0: "green", 0.9: "green", 0.8: "gold",
                   0.7: "orange", 0.6: "green", 0.5: "orange", 0.4: "orange",
                   0.3: "red", 0.2: "red", 0.1: "red", 0.0: "darkred"}
    
    ws1 = bouncing.Wall(200, 0, 100, 0, 2.0, False) # 2.0
    ws2 = bouncing.Wall(0, 0, 400, 90, 0.9, False) # 0.9
    ws3 = bouncing.Wall(400, 0, 400, 90, 0.9, False) # 0.9
    ws4 = bouncing.Wall(0, 400, 400, 0, 0.9, False) # 0.9
    w5 = bouncing.Wall(190, 50, 120, 150, 1, True) # 1
    w6 = bouncing.Wall(150, 100, 200, 30, 1, True) # 0.9
    w7 = bouncing.Wall(200, 150, 120, 150, 1, True) # 0.9
    w8 = bouncing.Wall(150, 200, 200, 30, 1, True) # 0.9
    w9 = bouncing.Wall(300, 50, 150, 30, 0.8, False) # 0.8
    w10 = bouncing.Wall(250, 100, 130, 100, 1.5, True) # 0.9
    w11 = bouncing.Wall(100, 100, 200, 85, 2.0, True) # 0.9
    w12 = bouncing.Wall(310, 360, 50, 40, 1.0, True) # 0.9
    ws5 = bouncing.Wall(300, 0, 100, 0, 0.4, False) # 0.4
    ws6 = bouncing.Wall(0, 0, 200, 0, 0.1, False) # 0.1
    walls = [ws1, ws2, ws3, ws4, w5, w6, w7, w8, w9, w10, w11, w12, ws5, ws6]
    """
    # Random walls --------------------------------------- #
    ws1 = bouncing.Wall(200, 0, 100, 0, 2.0, False) # 2.0
    ws2 = bouncing.Wall(0, 0, 400, 90, 0.9, False) # 0.9
    ...
    for rwi in range(rand_walls):
        walls.append(bouncing.Wall(np.random.randint(10, 300), np.random.randint(10, 300), np.random.randint(10, 100), np.random.randint(0, 180), np.around(0.1 * np.random.randint(0, 10), 1), True))
    # Random walls end------------------------------------ #
    """
    lw = 2
    root = tk.Tk()
